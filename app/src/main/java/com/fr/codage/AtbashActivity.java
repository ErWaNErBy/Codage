...
public class AtbashActivity extends AppCompatActivity {
    ...
    public void atbash(String mess){
        ...
        for (int i = 0; i < mess.length(); i++) {   // Analyse chaque caractère du message saisi
            ...
            if(reverseCodePoint > 127) {                                  // Si le décimal du caractère inversé est supérieur à 127 (c'est-à-dire, si c'est un caractère de la table ASCII étendue) ..
                String hexaOfExtendChar = "\\"+EXTENDEDS[reverseCodePoint-128];   // Récupère l'hexa du caractère inversé ( de la forme \0x**** )
                if(DECIMALS[reverseCodePoint-128] > 255) {	                      // Si le caratère inversé n'est pas affichable ..
                    ...
                    String extendCharacter = hexToAscii(EXTENDEDS[reverseCodePoint-128].substring(2) );     // On éxécute la fonction hexToAscii pour avoir le caratère inversé via sont hexa
                    messEncrypt.append(extendCharacter);                                                    // On ajoute dans le message résultat le caracère inversé de la table ASCII étendue
                }
            } else {                                                       // Sinon (c'est un caractère de la table ASCII normal) ..
                String normalCharacter = Character.toString( (char) (reverseCodePoint) );                                    // On récupère le caractère inversé avec son décimal
                String hexaOfNormalChar = "\\0x"+ Integer.toHexString( reverseCodePoint | 0x10000).substring(1);          // On récupère l'hexa du caractère inversé ( de la forme \0x**** )
                messEncrypt.append(normalCharacter.replaceAll("\\p{C}", "\\"+hexaOfNormalChar) );         // On ajoute dans le message résultat le caracère inversé de la table ASCII normal et si il n'est pas affichable on ajoute à la place son hexa
            }
        }
